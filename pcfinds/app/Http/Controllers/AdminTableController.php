...
class AdminTableController extends Controller
{
    ...
    // Admin Accounts CRUD in Admin Account
    public function admin_account_table()
    {
        // Fetch accounts with role 2 (admin)
        $accounts = Account::where('role', 2)->get();

        // Return the respective views for admin and customer
    ...
    // Customer Accounts CRUD in Admin Account
    public function customer_account_table()
    {
        // Fetch accounts with role 2 (admin)
        $customer_accounts = Account::where('role', 1)->get();

        // Return the respective views for admin and customer
        return view('content.customer_table', compact('customer_accounts'));
    }

    public function edit_customer_account_table($id)
    {
        $customer_account = Account::findOrFail($id);
        return view('content.edit_customer', compact('customer_account'));
    }

    public function update_customer_account_table(Request $request, $id)
    {
        $validator = Validator::make($request->all(), [
            'first_name' => 'required|string|max:50',
            'last_name' => 'required|string|max:50',
        ...
        // Find the account and update it with the sanitized data.
        $customer_account = Account::findOrFail($id);
        $customer_account->update($sanitized_data);
